...
def teacher_tab_attendance_records():
    ...
    for r in records:
        ...
        data.append({
            ...
            "Time": datetime.fromisoformat(ts).strftime("%Y-%m-%d %T:%M %p") if ts else "N'A",
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username = st.text_input("Enter username",placeholder='@arnav')
...
def teacher_screen_register():
    ...
    st.space()
    teacher_username = st.text_input("Enter username",placeholder='@arnav')
    tecaher_name = st.text_input("Enter name",placeholder='Arnav')
    ...
    with btnc1:
        if st.button('Register',icon=':material/passkey:',shortcut='control+enter',width='stretch'):
            success,message = register_teacher(teacher_username,tecaher_name,teacher_pass,teacher_pass_confirm)
            if success:
                st.success(message)
                import time
                time.sleep(2)
